...
namespace Course.Infrastructure.Implements
{
		...
		ICommandRepository<LessonQuiz> _lessonQuizRepository) : ICourseService
	{
		...
		public async Task<CreateCourseResponse> CreateAsync(CreateCourseDto dto, CancellationToken ct = default)
		{
			...
			foreach (var (moduleId, quizDto) in pendingModuleQuizzes)
			{
				var payload = ToQuizCourseInsertEvent(
					currentUser.Email,
					"Module Quiz",
					$"Auto-generated by CourseService | CourseId={course.CourseId} | ModuleId={moduleId}",
					quizDto
				);
			...
			foreach (var (moduleId, lessonId, quizDto) in pendingLessonQuizzes)
			{
				var payload = ToQuizCourseInsertEvent(
					currentUser.Email,
					"Lesson Quiz",
					$"Auto-generated by CourseService | CourseId={course.CourseId} | ModuleId={moduleId} | LessonId={lessonId}",
					quizDto
				);
		...
		private static QuizCourseInsertEvent ToQuizCourseInsertEvent(
			string userEmail,
			string title,        // ví dụ: "Module Quiz" | "Lesson Quiz"
			string description,  // ví dụ: "Auto-generated by CourseService"
			CreateQuizDto q)
		{

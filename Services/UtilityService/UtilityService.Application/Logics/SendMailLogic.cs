...
public static class SendMailLogic
{
    ...
    /// <param name="title"></param>
    /// <param name="body"></param>
    public static void Send(string mailAddress, string title, string body, ICommandRepository<UtilityService.Domain.Models.Systemconfig> systemConfigRepository)
    {
        var configs = systemConfigRepository.Find().ToList();
        // The SMTP server is obtained from the system configuration (SYSTEM_CONFIG).
        var mailSmtp = configs.Find(c => c.Id == SystemConfig.MailSmtp)?.Value;
        // The sender of the email is obtained from the system configuration (SYSTEM_CONFIG).
        var mailFrom = configs.Find(c => c.Id == SystemConfig.MailFrom)?.Value;
    ...
    /// <param name="detailErrorList"></param>
    /// <returns></returns>
    public static async Task<bool> SendMail(Emailtemplate emailTemplate, string mailAddress, ICommandRepository<Systemconfig> systemConfigRepository, List<DetailError> detailErrorList)
    {
        if (string.IsNullOrEmpty(emailTemplate.Title))
        {
            detailErrorList.Add(new DetailError
        ...
        {
            // Send mail
            Send(mailAddress, emailTemplate.Title, emailTemplate.Body, systemConfigRepository);
            return true;
        }
        catch (Exception e)
        {

        ...
        ICommandRepository<Subject> _subjectCommandRepository) : ICourseService
    {
        ...
        public async Task<GetCoursesResponse> GetAllAsync(PaginationRequest pagination, CourseQuery? query = null, CancellationToken ct = default)
        {
            ...
            var teacherResponse = await _teacherNameClient.GetResponse<GetTeacherNamesEventResponse>(
                new GetTeacherNamesEvent(teacherIds), ct);

            var teacherLookup = teacherResponse.Message.Response
                .ToDictionary(t => t.TeacherId, t => t.DisplayName);

            items = items.Select(i =>
            {
                teacherLookup.TryGetValue(i.TeacherId, out var teacherName);
                return i with { TeacherName = teacherName ?? string.Empty };
            }).ToList();
        ...
        public async Task<GetSuggestedCoursesEventResponse> GetSuggestedCoursesAsync(GetSuggestedCoursesStudentServiceDto request, CancellationToken ct = default)
        {
            ...
            {
                SuggestedCourseType.Harder => filtered.Where(c => c.Level.HasValue && c.Level.Value > request.CurrentLevel),
                SuggestedCourseType.Easier => filtered.Where(c => c.Level.HasValue && c.Level.Value < request.CurrentLevel),
                _ => filtered
            };

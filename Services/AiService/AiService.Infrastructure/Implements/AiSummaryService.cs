...
namespace AiService.Infrastructure.Implements
{
    ...
    ) : IAiSummaryService
    {
        public async Task<AiSummaryResponse> FeedBackCourseByAI(AiSummaryRequest req, CancellationToken ct)
        {
            ...
            var lowLessonScopeIds = new HashSet<Guid>(lowLessons.Select(x => x.scopeId));
            var lowModuleScopeIds = new HashSet<Guid>(lowModules.Select(x => x.scopeId));

            var lowLessonEvals = lessonAll.Where(e => lowLessonScopeIds.Contains(e.ScopeId)).ToList();
            var lowModuleEvals = moduleAll.Where(e => lowModuleScopeIds.Contains(e.ScopeId)).ToList();
            ...
            var topModuleNames = lowLessons
                .Select(x => lessonModuleMap.TryGetValue(x.scopeId, out var m) ? m : "—")
                .GroupBy(n => n)
            ...
            var payload = new
            {
                ...
                riskGroups = new
                {
                    lessons = lowLessons.Select(x => new
                    {
                        ...
                        module = lessonModuleMap.TryGetValue(x.scopeId, out var m) ? m : "—"
                    }),
